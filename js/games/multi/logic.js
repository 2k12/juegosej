window.GameRegistry['multi'] = class MultiGame extends window.GameEngine {
    constructor() {
        super('multi', 'Multiplicaciones Flash');
    }

    renderRound() {
        const num1 = this.getRandom(2, 10);
        const num2 = this.getRandom(2, 10);
    ...
    checkAnswer(selected, detail) {
        ...
        this.submitAnswer(isCorrect, `${detail.replace('*', '×')} = ${correct}`);
    }
}
